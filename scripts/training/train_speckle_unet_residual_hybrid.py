...
CROP_SIZE = 128
WORKER_BASE_SEED = 42
BEST_CHECKPOINT_NAME = "speckle_unet_residual_hybrid_best.pt"
LAST_CHECKPOINT_NAME = "speckle_unet_residual_hybrid_last.pt"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the Speckle U-Net denoiser with residual learning and hybrid loss.")
...
def predict_clean_from_residual(model: UNet, noisy: Tensor) -> tuple[Tensor, Tensor]:
    predicted_residual = model(noisy)
    predicted_clean = torch.clamp(noisy - predicted_residual, min=0.0, max=1.0)
    return predicted_residual, predicted_clean
...
def train_one_epoch(
    ...
    for noisy, clean in loader:
        ...
        with autocast_context(device, amp_enabled):
            _, predicted_clean = predict_clean_from_residual(model, noisy)
