...
CROP_SIZE = 128
WORKER_BASE_SEED = 42
BEST_CHECKPOINT_NAME = "periodic_unet_residual_hybrid_v3_best.pt"
LAST_CHECKPOINT_NAME = "periodic_unet_residual_hybrid_v3_last.pt"
...
class FrequencyLoss(nn.Module):
    """FFT magnitude loss to target periodic artefact suppression."""
...
class PeriodicV3HybridLoss(nn.Module):
    def __init__(self, l1_weight: float = 0.6, ssim_weight: float = 0.2, frequency_weight: float = 0.2) -> None:
        super().__init__()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the Periodic U-Net denoiser v3 with stronger periodic supervision.")
...
def run_epoch(
    model: UNet,
    loader: DataLoader,
    criterion: PeriodicV3HybridLoss,
    device: torch.device,
