...
CROP_SIZE = 128
WORKER_BASE_SEED = 42
BEST_CHECKPOINT_NAME = "periodic_unet_residual_hybrid_v2_best.pt"
LAST_CHECKPOINT_NAME = "periodic_unet_residual_hybrid_v2_last.pt"
...
class SSIMLoss(nn.Module):
    """Lightweight differentiable SSIM loss for RGB denoising."""
...
class ResidualHybridLoss(nn.Module):
    def __init__(self, l1_weight: float = 0.8, ssim_weight: float = 0.2) -> None:
        super().__init__()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the Periodic U-Net denoiser v2 with residual learning and hybrid loss.")
    parser.add_argument("--clean-dir", type=Path, default=Path("data/clean/bsd500"))
    parser.add_argument("--noisy-dir", type=Path, default=Path("data/synthetic/periodic"))
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            epochs_without_improvement = 0
            save_checkpoint(model, optimizer, scheduler, epoch, best_val_loss, best_checkpoint_path)
        else:
            epochs_without_improvement += 1

        save_checkpoint(model, optimizer, scheduler, epoch, best_val_loss, last_checkpoint_path)
