...
CROP_SIZE = 128
WORKER_BASE_SEED = 42
BEST_CHECKPOINT_NAME = "periodic_dual_branch_unet_best.pt"
LAST_CHECKPOINT_NAME = "periodic_dual_branch_unet_last.pt"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the Periodic dual-branch residual U-Net with hybrid periodic suppression loss.")
    ...
    parser.add_argument("--frequency-base-features", type=int, default=16)
...
def build_model(device: torch.device, args: argparse.Namespace) -> PeriodicDualBranchResidualUNet:
    return PeriodicDualBranchResidualUNet(
        rgb_in_channels=3,
        freq_in_channels=1,
        out_channels=3,
        base_features=args.base_features,
        frequency_base_features=args.frequency_base_features,
    ).to(device)
...
def run_epoch(
    ...
    for noisy, clean in loader:
        ...
        x_freq = rgb_to_normalized_fft_log_magnitude(noisy)
        if training:
            optimizer.zero_grad(set_to_none=True)

        with torch.amp.autocast(device_type=device.type, enabled=amp_enabled):
            predicted_residual = model(noisy, x_freq)
...
def print_sanity_batch(loader: DataLoader) -> None:
    noisy, clean = next(iter(loader))
    x_freq = rgb_to_normalized_fft_log_magnitude(noisy)
    print(f"Sanity batch noisy shape: {tuple(noisy.shape)}")
    print(f"Sanity batch clean shape: {tuple(clean.shape)}")
    print(f"Sanity batch frequency map shape: {tuple(x_freq.shape)}")
...
def main() -> None:
    ...
    print_sanity_batch(train_loader)
