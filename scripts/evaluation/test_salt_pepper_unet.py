...
SEED = 42
CHECKPOINT_PATH = Path("models/denoisers/salt_pepper/salt_pepper_unet_best.pt")
CLEAN_DIR = Path("data/clean/bsd500")
NOISY_DIR = Path("data/synthetic/salt_pepper")
OUTPUT_DIR = Path("outputs/salt_pepper_unet_test")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate the active Salt & Pepper U-Net checkpoint.")
...
def collect_pairs(clean_dir: Path, noisy_dir: Path) -> list[PairedSample]:
    ...
    if not paired_samples:
        raise FileNotFoundError("No valid salt_pepper clean/noisy pairs were found.")
...
def evaluate_dataset(args: argparse.Namespace) -> None:
    ...
    for sample in tqdm(test_samples, desc="Testing Salt & Pepper UNet", unit="image"):
...
def evaluate_single_image(args: argparse.Namespace) -> None:
    ...
    output_path = output_dir / f"{image_path.stem}_salt_pepper_unet.png"
