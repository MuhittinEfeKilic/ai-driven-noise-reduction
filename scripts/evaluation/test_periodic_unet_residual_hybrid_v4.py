...
BATCH_SIZE = 12
NUM_WORKERS = 2
CHECKPOINT_PATH = Path("archive_models/denoisers/periodic/periodic_unet_residual_hybrid_best.pt")
CLEAN_DIR = Path("data/clean/bsd500")
NOISY_DIR = Path("data/synthetic/periodic_v3")
OUTPUT_DIR = Path("outputs/periodic_residual_hybrid_v4_test")
...
class PeriodicV4TestDataset(Dataset[tuple[Tensor, Tensor, str]]):
...
def create_test_loader() -> DataLoader:
    ...
    dataset = PeriodicV4TestDataset(test_samples)
    return DataLoader(
        dataset,
...
def main() -> None:
    ...
    for noisy_batch, clean_batch, names in tqdm(loader, desc="Testing Periodic Residual Hybrid V4", unit="batch"):
