...
SEED = 42
CHECKPOINT_PATH = Path("models/denoisers/gaussian/gaussian_unet_best.pt")
CLEAN_DIR = Path("data/clean/bsd500")
NOISY_DIR = Path("data/synthetic/gaussian")
OUTPUT_DIR = Path("outputs/gaussian_unet_test")
GAUSSIAN_PATTERN = re.compile(r"^(?P<base>.+?)_g\d+_s\d+$")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate the active Gaussian U-Net checkpoint.")
...
def noisy_to_clean_stem(noisy_stem: str) -> str:
    match = GAUSSIAN_PATTERN.match(noisy_stem)
...
def collect_pairs(clean_dir: Path, noisy_dir: Path) -> list[PairedSample]:
    ...
    if not paired_samples:
        raise FileNotFoundError("No valid gaussian clean/noisy pairs were found.")
...
def evaluate_dataset(args: argparse.Namespace) -> None:
    ...
    for sample in tqdm(test_samples, desc="Testing Gaussian UNet", unit="image"):
...
def evaluate_single_image(args: argparse.Namespace) -> None:
    ...
    output_path = output_dir / f"{image_path.stem}_gaussian_unet.png"
